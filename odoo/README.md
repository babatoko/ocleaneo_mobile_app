...
|----------|--------|------|
| `OCLEANEO_MOBILE_CORS_ORIGIN` | `https://localhost` | Origine autorisée en CORS sur les routes `/api/mobile/*`. Le défaut cible l'app native packagée : la WebView Android de Capacitor est un vrai moteur Chromium et applique CORS comme n'importe quel navigateur (origine par défaut `https://localhost`, voir `CapConfig.java` — inchangée par `capacitor.config.json`). Il faut le régler explicitement sur `http://127.0.0.1:5173` pour le serveur de dev Vite, ou sur l'origine du site pour un déploiement **PWA**. Un `warning` est journalisé au démarrage quand la variable n'est pas définie, pour que l'oubli se voie dans le log plutôt qu'en mur de requêtes bloquées — voir la docstring de `tools/mobile_auth.py` (dépôt `ocleaneo`) pour l'historique : un défaut pointant à tort sur l'origine Vite a longtemps cassé silencieusement la connexion de l'app native en production, indiscernable côté app d'une simple coupure réseau. |
