...
export const usePointageStore = defineStore('pointage', {
  ...
  actions: {
    ...
    async clockWithTag(uid: string): Promise<void> {
      ...
      const created = await this.postEntry(type, { chantierId: site.id, shiftId: shift?.id, position, geo });
      hapticSuccess();
      ...
      if (type === 'out') {
        ...
        // comportement ("done").
        if (created?.shift_status) {
          await usePlanningStore().markShiftDone(site.id, created.shift_status);
        } else {
