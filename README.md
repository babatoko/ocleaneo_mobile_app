...
- **Migration complète des mises en page métier** — les cartes de vacation, la grille de stock, la timeline de tournée, les listes (historique, inventaire, réglages…) sont aussi de vrais composants Ionic (`ion-card`, `ion-list`/`ion-item`/`ion-item-group`/`ion-item-divider`, `ion-chip`, `ion-badge`, `ion-avatar`, `ion-progress-bar`, `ion-grid`/`ion-row`/`ion-col`…), plutôt que des `<div>` habillées en CSS custom. Un principe d'accessibilité a été appliqué systématiquement à chaque conversion : ne jamais imbriquer un élément interactif (`<button>`/`<a>`) dans un autre — `ion-item button` (toute la ligne cliquable) uniquement quand la ligne n'a pas d'action secondaire, sinon un `ion-item` non-bouton hébergeant un vrai `<button>` pour l'action principale et les actions secondaires en `ion-button` frères dans un slot nommé. Chaque composant Ionic reçoit ses propres surcharges de variables CSS (`--background`, `--color`, `--border-radius`, `--box-shadow: none`…) pour rester fidèle à l'identité visuelle Ocleaneo plutôt que d'adopter le chrome Ionic par défaut.
...
**Mode hors ligne** (`services/offlineQueue.ts`) — si `POST /time-entries` échoue par coupure réseau (pas une erreur métier — `!error.response`), le pointage est mis en file d'attente locale persistante (`@capacitor/preferences`) au lieu d'être perdu, avec une mise à jour optimiste immédiate de l'écran (marquée « en attente de synchronisation »). La file est rejouée dans l'ordre dès que `@capacitor/network` signale un retour de connexion ou que `@capacitor/app` signale un retour au premier plan (`watchConnectivity()`), en s'arrêtant à la première entrée qui échoue encore pour ne pas désynchroniser l'alternance arrivée/départ. Chaque pointage porte un `clientRef` (généré une fois dans `stores/pointage.ts`, `postEntry()`, avant même la première tentative) réutilisé tel quel à chaque rejeu : si une tentative réussit côté serveur mais que la réponse se perd (coupure au mauvais moment), le client la revoit comme une panne réseau et la remet en file — sans cette clé, le rejeu créerait un doublon silencieux plutôt que d'être reconnu comme le même pointage. `stores/chantiers.ts` met aussi en cache la liste des chantiers (même mécanisme) pour que le badge reste reconnaissable même hors ligne. La file vaut aussi pour la PWA : `@capacitor/preferences` retombe sur `localStorage` dans le navigateur, et `@capacitor/network`/`@capacitor/app` ont une implémentation web (`online`/`offline`, `visibilitychange`) — il n'y a donc pas de garde « natif uniquement » sur ce chemin. *Limite* : sans avoir jamais eu de réseau au moins une fois dans la session (chantiers jamais mis en cache), un badge ne peut pas être matché — couvre le cas réel (perte de réseau sur site après une ouverture d'app le matin), pas un premier lancement hors ligne.
...
**Permission de localisation native** — ni le manifeste Android ni l'Info.plist iOS ne déclaraient de permission de localisation, alors que `navigator.geolocation.getCurrentPosition()` est appelée ici et par la Tournée (voir plus bas). Aucune dépendance `@capacitor/geolocation` : l'appel passe par l'API de géolocalisation du WebView, dont le pont natif Capacitor (`BridgeWebChromeClient`) demande la permission au système au moment de l'appel. Sans la déclaration dans le manifeste, Android **refuse silencieusement** — la boîte de dialogue système ne s'affiche même pas — et l'anti-fraude ci-dessus, tout comme le point de départ de la Tournée, se dégradaient sans un mot. Sur iOS, l'absence de `NSLocationWhenInUseUsageDescription` dans l'Info.plist est le schéma de plantage documenté par Apple pour une app WKWebView appelant une API de localisation sans description d'usage. Corrigé : `ACCESS_COARSE_LOCATION`/`ACCESS_FINE_LOCATION` dans `android/app/src/main/AndroidManifest.xml`, `NSLocationWhenInUseUsageDescription` dans `ios/App/App/Info.plist`.

**Sauvegarde automatique Android désactivée** (`android:allowBackup="false"` dans `AndroidManifest.xml`) — Android sauvegarde et restaure par défaut les `SharedPreferences` de l'app (donc `@capacitor/preferences`) sur le compte Google du salarié, y compris à travers une désinstallation complète. Repéré sur un « premier lancement après installation » qui ne l'était pas vraiment : le backend actif (`ocleaneo_data_provider`) et l'URL du serveur REST (`ocleaneo_api_base_url` — un reliquat d'avant l'existence du sélecteur odoo/rest/démo, voir § Architecture backend-agnostique) avaient été restaurés depuis une ancienne installation, avec REST pointé sur l'instance Odoo réelle — un mélange de contrats (REST/JSON générique vs JSON-RPC Odoo) qui fait échouer toute connexion avec un message « Pas de connexion » trompeur. Pour un outil interne lié à un seul backend, restaurer d'anciens réglages locaux après une réinstallation n'est jamais souhaitable.
...
**Contrat Odoo à prévoir** pour tout ce qui précède : `POST /time-entries` doit accepter `recordedAt` (horodatage réel du scan, capturé côté client — important pour l'exactitude d'un pointage mis en file d'attente et synchronisé plus tard), `outOfRange` (booléen, méta-donnée de revue) et **`clientRef`** (chaîne opaque, générée côté client, à traiter comme une clé d'idempotence : un appel qui la reçoit une deuxième fois — après une réponse perdue puis un rejeu depuis la file hors ligne — doit renvoyer le pointage déjà créé plutôt qu'en créer un doublon, typiquement via une contrainte d'unicité sur cette colonne) ; `type` doit accepter `pause_start`/`pause_end` en plus de `in`/`out` ; un nouvel endpoint `GET /time-entries/mine?from&to` doit renvoyer la liste des pointages sur une période. `providers/MockProvider.ts` implémente ce dédoublonnage (voir `createTimeEntry()`) — la référence de ce que le contrat attend côté serveur.
